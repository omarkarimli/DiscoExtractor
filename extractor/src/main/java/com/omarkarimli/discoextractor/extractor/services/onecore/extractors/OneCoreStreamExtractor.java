...
import com.omarkarimli.discoextractor.extractor.exceptions.LiveNotStartException;
import com.omarkarimli.discoextractor.extractor.exceptions.OneCoreMusicPremiumContentException;
import com.omarkarimli.discoextractor.extractor.exceptions.PaidContentException;
...
import com.omarkarimli.discoextractor.extractor.stream.Frameset;
import com.omarkarimli.discoextractor.extractor.stream.Stream;
import com.omarkarimli.discoextractor.extractor.stream.StreamExtractor;
import com.omarkarimli.discoextractor.extractor.stream.StreamSegment;
...
import com.omarkarimli.discoextractor.extractor.utils.Pair;
...
import static com.omarkarimli.discoextractor.extractor.services.onecore.OneCoreParsingHelper.*;
import static com.omarkarimli.discoextractor.extractor.services.onecore.OneCoreService.getTempLocalization;
...
public class OneCoreStreamExtractor extends StreamExtractor {
    ...
    public OneCoreStreamExtractor(final StreamingService service, final LinkHandler linkHandler, WatchDataCache watchDataCache) {
...
//        }
    ...
    public DateWrapper getUploadDate() throws ParsingException {
        ...
        return new DateWrapper(OneCoreParsingHelper.parseDateFrom(textualUploadDate), true);
    }
    ...
    public String getUploaderUrl() throws ParsingException {
        ...
        if (!isNullOrEmpty(uploaderId)) {
            return OneCoreChannelLinkHandlerFactory.getInstance().getUrl("channel/" + uploaderId);
        }
    ...
    public boolean isUploaderVerified() throws ParsingException {
        return OneCoreParsingHelper.isVerified(
    ...
    public List<SubtitlesStream> getSubtitles(final MediaFormat format) throws ParsingException {
        ...
        String contentLanguage = ServiceList.OneCore.getContentLanguage().getLocalizationCode();
        ...
        if (translatableSubtitle != null && !hasFoundContentLanguage && ServiceList.OneCore.getShowAutoTranslatedSubtitles()) {
    ...
    public MultiInfoItemsCollector getRelatedItems() throws ExtractionException {
        ...
        try {
                    ...
                    .map(result -> {
                        if (result.has("compactVideoRenderer")) {
                            ...
                            if ("LOCKUP_CONTENT_TYPE_PLAYLIST".equals(
                                ...
                                return new OneCoreMixOrPlaylistLockupInfoItemExtractor(
                                        lockupViewModel);
                            }
                            else if ("LOCKUP_CONTENT_TYPE_VIDEO".equals(
                                    lockupViewModel.getString("contentType"))){
                                return new OneCoreLockupStreamInfoItemExtractor(lockupViewModel, timeAgoParser);
                            }
                        }
                        return null;
                    })
            ...
            if (ServiceList.OneCore.getFilterTypes().contains("related_item")) {
    ...
    public void onFetchPage(@Nonnull final Downloader downloader)
        ...
        CancellableCall webPageCall = OneCoreParsingHelper.getWebPlayerResponse(
        ...
        if (StringUtils.isBlank(ServiceList.OneCore.getTokens())) {
       ...
       CancellableCall nextDataCall = getJsonPostResponseAsync(NEXT, body, localization, new Downloader.AsyncCallback() {
        ...
        do {
            if (((StringUtils.isBlank(ServiceList.OneCore.getTokens()) && androidCall.isFinished())
                    || (StringUtils.isNotBlank(ServiceList.OneCore.getTokens()) && webCall.isFinished() && tvCall.isFinished())) &&
                    webPageCall.isFinished() && nextDataCall.isFinished()) {
                break;
            }
        } while (TimeUnit.NANOSECONDS.toSeconds(System.nanoTime() - startTime) <= ServiceList.OneCore.getLoadingTimeout());

        if (((StringUtils.isBlank(ServiceList.OneCore.getTokens()) && androidStreamingData == null)
                || ((StringUtils.isNotBlank(ServiceList.OneCore.getTokens()) && webStreamingData == null && tvHtml5SimplyEmbedStreamingData == null)))
    ...
    public static JsonObject checkPlayabilityStatus(@Nonnull JsonObject playabilityStatus, String videoId)
        ...
        if ((status.equalsIgnoreCase("unplayable") || status.equalsIgnoreCase("error"))
            ...
            if (reason.contains("Music Premium")) {
                throw new OneCoreMusicPremiumContentException();
            }
    ...
    private CancellableCall fetchWebJsonPlayer(@Nonnull final ContentCountry contentCountry,
        ...
        return getLoggedJsonPostResponseAsync(PLAYER,
                createWebEmbedPlayerBody(localization,
                        contentCountry,
                        videoId,
                        OneCoreJavaScriptPlayerManager.getSignatureTimestamp(videoId),
    ...
    private java.util.stream.Stream<ItagInfo> getStreamsFromStreamingDataKey(
        ...
        String contentLanguage = ServiceList.OneCore.getAudioLanguage();
    ...
    private void batchDeobfuscateItagUrls(@Nonnull final String videoId,
        ...
        for (int i = 0; i < itagInfoList.size(); i++) {
            ...
            final String throttlingParam =
                OneCoreThrottlingParameterUtils.getThrottlingParameterFromStreamingUrl(url);
        ...
        // Make batch API call
        final OneCoreApiDecoder.BatchDecodeResult result =
            OneCoreJavaScriptPlayerManager.deobfuscateBatch(
                videoId,
    ...
    public List<MetaInfo> getMetaInfo() throws ParsingException {
        return OneCoreParsingHelper.getMetaInfo(nextResponse
                .getObject("contents")
                .getObject("twoColumnWatchNextResults")
                .getObject("results")
                .getObject("results")
