...
public final class OneCorePlaylistLinkHandlerFactory extends ListLinkHandlerFactory {

    private static final OneCorePlaylistLinkHandlerFactory INSTANCE =
            new OneCorePlaylistLinkHandlerFactory();

    private OneCorePlaylistLinkHandlerFactory() {
    }

    public static OneCorePlaylistLinkHandlerFactory getInstance() {
        return INSTANCE;
    }
    ...
    public ListLinkHandler fromUrl(final String url) throws ParsingException {
        try {
            final URL urlObj = Utils.stringToURL(url);
            final String listID = Utils.getQueryValue(urlObj, "list");
            if (listID != null && OneCoreParsingHelper.isOneCoreMixId(listID)) {
                String videoID = Utils.getQueryValue(urlObj, "v");
                if (videoID == null) {
                    videoID = OneCoreParsingHelper.extractVideoIdFromMixId(listID);
                }
