package org.schabi.newpipe.extractor.services.onecore.search.filter;

import org.schabi.newpipe.extractor.search.filter.Filter;
import org.schabi.newpipe.extractor.search.filter.FilterGroup;
import org.schabi.newpipe.extractor.search.filter.FilterItem;
import org.schabi.newpipe.extractor.search.filter.SearchFiltersBase;
import org.schabi.newpipe.extractor.services.onecore.search.filter.protobuf.DateFilter;
import org.schabi.newpipe.extractor.services.onecore.search.filter.protobuf.Features;
import org.schabi.newpipe.extractor.services.onecore.search.filter.protobuf.LenFilter;
import org.schabi.newpipe.extractor.services.onecore.search.filter.protobuf.SortOrder;
import org.schabi.newpipe.extractor.services.onecore.search.filter.protobuf.TypeFilter;
...
public final class OneCoreFilters extends SearchFiltersBase {
     ...
     */
    public String buildSortFilterOneCore(final TypeFilter contentFilter) {
        final OneCoreSearchSortFilter.Builder builder = new OneCoreSearchSortFilter.Builder();
        builder.setTypeFilter(contentFilter);

        for (final FilterItem sortItem : selectedSortFilter) {
            if (sortItem instanceof OneCoreSortOrderSortFilterItem) {
                final SortOrder sortOrder = ((OneCoreSortOrderSortFilterItem) sortItem).get();
                if (null != sortOrder) {
                    builder.setSortOrder(sortOrder);
                }
            } else if (sortItem instanceof OneCoreDateSortFilterItem) {
                final DateFilter dateFilter = ((OneCoreDateSortFilterItem) sortItem).get();
                if (null != dateFilter) {
                    builder.setDateFilter(dateFilter);
                }
            } else if (sortItem instanceof OneCoreLenSortFilterItem) {
                final LenFilter lenFilter = ((OneCoreLenSortFilterItem) sortItem).get();
                if (null != lenFilter) {
                    builder.setLenFilter(lenFilter);
                }
            } else if (sortItem instanceof OneCoreFeatureSortFilterItem) {
                final Features feature = ((OneCoreFeatureSortFilterItem) sortItem).get();
                if (null != feature) {
        ...
        if (selectedContentFilter != null && !selectedContentFilter.isEmpty()) {
            ...
            if (contentFilterTypeMap.containsKey(filterItem)) {
                ...
                if (selectedSortFilter != null) {
                    sp = buildSortFilterOneCore(cf);
                } else {
                    sp = "";
                }

                if (filterItem instanceof MusicOneCoreContentFilterItem) {
                    final MusicOneCoreContentFilterItem realFilterItem =
                            (MusicOneCoreContentFilterItem) filterItem;
                    try {
                ...
                } else if (filterItem instanceof OneCoreContentFilterItem) {
                    final OneCoreContentFilterItem realFilterItem =
                            (OneCoreContentFilterItem) filterItem;
                    try {
    ...
    protected void init() {
        ...
        final int contentFilterAllId = addFilterItem(
                new OneCoreContentFilterItem(ALL), null);
        final int contentFilterVideos = addFilterItem(
                new OneCoreContentFilterItem(VIDEOS), TypeFilter.video);
        final int contentFilterChannels = addFilterItem(
                new OneCoreContentFilterItem(CHANNELS), TypeFilter.channel);
        ...
        addContentFilter(builder.createSortGroup("YouTube Music", true, new FilterItem[]{
                builder.getFilterForId(addFilterItem(
                        new MusicOneCoreContentFilterItem(MUSIC_SONGS,
                                "Eg-KAQwIARAAGAAgACgAMABqChAEEAUQAxAKEAk%3D"
                        ), null)),
                builder.getFilterForId(addFilterItem(
                        new MusicOneCoreContentFilterItem(MUSIC_VIDEOS,
                                "Eg-KAQwIABABGAAgACgAMABqChAEEAUQAxAKEAk%3D"
                        ), null)),
                builder.getFilterForId(addFilterItem(
                        new MusicOneCoreContentFilterItem(MUSIC_ALBUMS,
                                "Eg-KAQwIABAAGAEgACgAMABqChAEEAUQAxAKEAk%3D"
                        ), null)),
                builder.getFilterForId(addFilterItem(
                        new MusicOneCoreContentFilterItem(MUSIC_PLAYLISTS,
                                "Eg-KAQwIABAAGAAgACgBMABqChAEEAUQAxAKEAk%3D"
                        ), null)),
                builder.getFilterForId(addFilterItem(
                        new MusicOneCoreContentFilterItem(MUSIC_ARTISTS,
        ...
        final int filterIdSortByRelevance = builder.addSortItem(
                new OneCoreSortOrderSortFilterItem("sort_relevance", SortOrder.relevance));
        final int filterIdSortByRating = builder.addSortItem(
                new OneCoreSortOrderSortFilterItem("sort_rating", SortOrder.rating));
        final int filterIdSortByDate = builder.addSortItem(
                new OneCoreSortOrderSortFilterItem("sort_publish_time", SortOrder.date));
        final int filterIdSortByViews = builder.addSortItem(
                new OneCoreSortOrderSortFilterItem("sort_view", SortOrder.views));

        /* 'Date' filter items */
        final int filterIdDateAll = builder.addSortItem(
                new OneCoreDateSortFilterItem("All", null));
        final int filterIdDateLastHour = builder.addSortItem(
                new OneCoreDateSortFilterItem("Hour", DateFilter.hour));
        final int filterIdDateLastDay = builder.addSortItem(
                new OneCoreDateSortFilterItem("Day", DateFilter.day));
        final int filterIdDateLastWeek = builder.addSortItem(
                new OneCoreDateSortFilterItem("Week", DateFilter.week));
        final int filterIdDateLastMonth = builder.addSortItem(
                new OneCoreDateSortFilterItem("Month", DateFilter.month));
        final int filterIdDateLastYear = builder.addSortItem(
                new OneCoreDateSortFilterItem("Year", DateFilter.year));

        /* 'Duration' filter items */
        final int filterIdDurationAll = builder.addSortItem(
                new OneCoreLenSortFilterItem("All", null));
        final int filterIdDurationShort = builder.addSortItem(
                new OneCoreLenSortFilterItem("Short", LenFilter.duration_short));
        final int filterIdDurationLong = builder.addSortItem(
                new OneCoreLenSortFilterItem("Long", LenFilter.duration_long));

        /* 'features' filter items */
        final int filterIdFeatureHd = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("HD", Features.is_hd));
        final int filterIdFeatureSubtitles = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("Subtitles", Features.subtitles));
        final int filterIdFeatureCcommons = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("Ccommons", Features.ccommons));
        final int filterIdFeature3d = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("3d", Features.is_3d));
        final int filterIdFeatureLive = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("Live", Features.live));
        final int filterIdFeaturePurchased = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("Purchased", Features.purchased));
        final int filterIdFeature4k = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("4k", Features.is_4k));
        final int filterIdFeature360 = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("360°", Features.is_360));
        final int filterIdFeatureLocation = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("Location", Features.location));
        final int filterIdFeatureHdr = builder.addSortItem(
                new OneCoreFeatureSortFilterItem("Hdr", Features.is_hdr));
    ...
    private static class OneCoreSortOrderSortFilterItem extends OneCoreSortFilterItem {
        private final SortOrder sortOrder;

        OneCoreSortOrderSortFilterItem(final String name, final SortOrder sortOrder) {
            super(name);
    ...
    private static class OneCoreDateSortFilterItem extends OneCoreSortFilterItem {
        private final DateFilter dateFilter;

        OneCoreDateSortFilterItem(final String name, final DateFilter dateFilter) {
            super(name);
    ...
    private static class OneCoreLenSortFilterItem extends OneCoreSortFilterItem {
        private final LenFilter lenFilter;

        OneCoreLenSortFilterItem(final String name, final LenFilter lenFilter) {
            super(name);
    ...
    private static class OneCoreFeatureSortFilterItem extends OneCoreSortFilterItem {
        private final Features feature;

        OneCoreFeatureSortFilterItem(final String name, final Features feature) {
            super(name);
    ...
    public static class OneCoreSortFilterItem extends FilterItem {

        public OneCoreSortFilterItem(final String name) {
            super(Filter.ITEM_IDENTIFIER_UNKNOWN, name);
        }
    }

    public static class OneCoreContentFilterItem extends OneCoreSortFilterItem {
        public final String searchUrl;
        protected String params;

        public OneCoreContentFilterItem(final String name) {
            super(name);
            this.searchUrl = SEARCH_URL;
            this.params = "";
        }

        public OneCoreContentFilterItem(final String name, final String searchUrl) {
            super(name);
    ...
    public static class MusicOneCoreContentFilterItem extends OneCoreContentFilterItem {
        public MusicOneCoreContentFilterItem(final String name, final String params) {
